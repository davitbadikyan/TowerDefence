		...
		for (const spritesheet in allSpritesheets) {
			if (!this.game.cache.checkImageKey(allSpritesheets[spritesheet].getName())) {
				...
				for (const option of Object.getOwnPropertyNames(allSpritesheets[spritesheet])) {
					if (option !== 'getName' && option !== 'getFrameWidth' && option !== 'getFrameHeight' && option !== 'getFrameMax'
						&& option !== 'getMargin' && option !== 'getSpacing' && option.includes('get')) {
						imageOption = option;
					}
				}

				this.game.load.spritesheet(
					allSpritesheets[spritesheet].getName(),
					allSpritesheets[spritesheet][imageOption](),
					allSpritesheets[spritesheet].getFrameWidth(),
					allSpritesheets[spritesheet].getFrameHeight(),
					allSpritesheets[spritesheet].getFrameMax(),
					allSpritesheets[spritesheet].getMargin(),
					allSpritesheets[spritesheet].getSpacing()
				);
			}
		}
	}
		...
		for (const atlas in allAtlases) {
			if (!this.game.cache.checkImageKey(allAtlases[atlas].getName())) {
				...
				if (dataOption === 'getXML') {
					this.game.load.atlasXML(allAtlases[atlas].getName(),
						allAtlases[atlas][imageOption](),
						allAtlases[atlas].getXML());
				} else if (dataOption === 'getJSONArray') {
					this.game.load.atlasJSONArray(allAtlases[atlas].getName(),
						allAtlases[atlas][imageOption](),
						allAtlases[atlas].getJSONArray());
				} else if (dataOption === 'getJSONHash') {
					this.game.load.atlasJSONHash(allAtlases[atlas].getName(),
						allAtlases[atlas][imageOption](),
						allAtlases[atlas].getJSONHash());
				}
			}
		}
	}
	...
	private static loadBitmapFonts() {
		...
		for (const font in allBitmapFonts) {
			if (!this.game.cache.checkBitmapFontKey(allBitmapFonts[font].getName())) {
				...
				this.game.load.bitmapFont(allBitmapFonts[font].getName(),
					allBitmapFonts[font][imageOption](),
					allBitmapFonts[font][dataOption]());
			}
		}
	}
